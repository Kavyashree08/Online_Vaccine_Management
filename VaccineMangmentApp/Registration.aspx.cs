                ...
                try
                {
                    ...
                    using (var ctx = new VaccineMasterEntities1())
                    {
                        if (!ctx.UserMasters.Any(x => x.EmailID == emailId || x.MobileNo == mobileNo))
                        {
                            ...
                            ctx.SaveChanges();
                            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Registerd Successfully')", true);
                            ErrorMsg.InnerText = "Registerd Successfully";
                        }
                        else
                        {
                            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Customer Already Registerd')", true);
                            ErrorMsg.InnerText = "Customer Already Registerd";
                        }
